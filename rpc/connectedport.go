...
func (port *ConnectedPort) bufferRunner() {
	...
	for port.localErr == nil {
		...
		if transferDebugEnabled() {
			tdBufferRunnerWrites.Add(1)
			tdAddBytes(&tdBufferRunnerBytes, n)
		}
	}
...
func (port *ConnectedPort) SendRemote(data []byte) (err error) {
	...
	port.srv.Call(func() {
		...
		if err == nil {
			if transferDebugEnabled() {
				tdSendRemoteCalls.Add(1)
				tdAddBytes(&tdSendRemoteBytes, len(data))
			}
			go func() {
...
func (port *ConnectedPort) SendLocal(data []byte) (err error) {
	...
	if port.remoteErr != nil {
		...
		if transferDebugEnabled() {
			tdSendLocalCalls.Add(1)
			tdAddBytes(&tdSendLocalBytes, len(data))
			tdRecordLocalBufferLen(port.localBuffer.Len())
		}
...
func (port *ConnectedPort) upgradeTLS(fn func(*E2EServer) error) error {
	if port.client == nil {
		err := fmt.Errorf("aborting TLS upgrade during client shutdown")
