...
func setRegisterError(err error) {
	registerErrMu.Lock()
	registerErr = err
	registerErrMu.Unlock()
}

func getRegisterError() error {
	registerErrMu.RLock()
	defer registerErrMu.RUnlock()
	return registerErr
}
...
func Register(onReady func(), onExit func()) error {
	...
	if !registerSystray() {
		...
		systrayExit = func() {}
		onExit()
		if err := getRegisterError(); err != nil {
			return err
		}
		return errors.New("systray: failed to initialize")
	}
	return nil
}
